#!/usr/bin/env python3
"""
Block Blast - Human Play GUI with Demonstration Recording

Built from scratch based on the original working blockblast/pygame_app.py
...
from blockblast.env import encode_action


# Colors (from original working game)
COLORS = {
    'background': (20, 24, 29),
    'grid_line': (40, 48, 58),
    'empty_cell': (30, 36, 44),
...
# Piece colors (from original working game)
PIECE_COLORS = [
    (139, 195, 74),   # Green
    (255, 193, 7),    # Amber
...
class HumanPlayGUI:
    """Human playable Block Blast with demonstration recording."""
    ...
    def __init__(self, window_width: int = 1000, window_height: int = 700):
        ...
        pygame.display.set_caption("Block Blast - Human Play (Recording)")
        ...
        # UI layout (same as original working version)
        self.board_size = 500
        ...
        print("\n" + "="*70)
        print("BLOCK BLAST - HUMAN DEMONSTRATION RECORDING")
        print("="*70)
        print("\nControls:")
        print("  - Click and drag pieces to the board")
        print("  - R: Restart game")
        print("  - ESC: Quit and save demonstrations")
        print("\nPlay 20-50 games for good training data!")
        print("="*70 + "\n")
    ...
    def handle_mouse_down(self, pos: Tuple[int, int]):
        ...
        if self.env.state and self.env.state.done:
            return
    ...
    def handle_mouse_up(self, pos: Tuple[int, int]):
        ...
        if self.preview_position is not None:
            ...
            # Encode the action (using correct function)
            ...
            # Apply the action
            self.obs, reward, terminated, truncated, info = self.env.step(action_id)
            
            # Record all successful placements (not based on reward, which can be negative)
            # The fact that step() succeeded means it was a valid move
            self.demonstrations.append((obs_before, action_id))
            
            # Check if game ended
            if terminated or truncated:
                print(f"Game Over! Score: {self.env.state.score}, Lines: {self.env.state.lines_cleared_total}")
    ...
    def get_piece_at_position(self, pos: Tuple[int, int]) -> Optional[int]:
        ...
        for i in range(len(self.env.state.available_pieces)):
    ...
    def get_board_position_from_mouse(self, pos: Tuple[int, int]) -> Optional[Tuple[int, int]]:
        ...
        # Get piece
        piece_id = self.env.state.available_pieces[self.selected_piece_idx]
    ...
    def render_header(self):
        ...
        # Title
        title = self.font_large.render("Block Blast - Recording", True, COLORS['text'])
        self.screen.blit(title, (self.board_x, 20))
        
        # Stats
        stats_text = f"Score: {state.score}  |  Moves: {state.moves}  |  Combo: {state.combo}x  |  Demos: {len(self.demonstrations)}"
    ...
    def render_placement_preview(self):
        ...
        valid = self.env.game.can_place_piece(
            self.env.state.board, piece, row, col
        )
    ...
    def render_game_over(self):
        ...
        score_text = self.font_medium.render(
            f"Final Score: {self.env.state.score}", 
            True, COLORS['text']
        )
